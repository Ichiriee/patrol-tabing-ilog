...
for i, (sh_name, beta) in enumerate(zip(SHIFTS, BETAS)):
    ...
    for j in range(len(VEHICLE_NAMES)):
        ...
        ax.text(bar_x, val + 0.2, f"{val:.1f}", ha='center', va='bottom', fontsize=8, fontweight='bold')
...
ax.set_ylim(0, t_cycle(D_km, SPEED, 1.4) * 1.2)  # Extra headroom for labels
