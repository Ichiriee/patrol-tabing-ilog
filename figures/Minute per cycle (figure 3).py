...
for i, (sh_name, beta) in enumerate(zip(SHIFTS, BETAS)):
    ...
    for j in range(len(VEHICLE_NAMES)):
        ...
        # Numerical labels on top of bars
        ax.text(bar_x, val + 0.1, f"{val:.1f}", ha='center', va='bottom', fontsize=8, fontweight='bold')
...
ax.set_ylim(0, t_cycle(D_km, SPEED, 1.4) * 1.25)  # Extra headroom for low-profile 30kph bars
ax.grid(axis='y', linestyle='--', alpha=0.3)

# --- 4. LEGEND (MOVED TO AVOID OBSTRUCTION) ---
# Legend moved above the plot area to prevent covering bars or text
ax.legend(handles=patches, loc="upper center", ncol=4, bbox_to_anchor=(0.5, 1.12),
          frameon=True, fontsize=10)

plt.tight_layout(rect=[0, 0.05, 1, 0.88])
plt.savefig("Minute_per_cycle_figure_3_30kph.png", dpi=150, bbox_inches="tight")
plt.show()
