...
# --- 3. CELL 11: CPP ANALYSIS - PAIRWISE DISTANCES ---

# Identification of odd-degree vertices for the Chinese Postman Problem
odd_verts = sorted([v for v in G.nodes() if G.degree(v) % 2 == 1])

print("\n" + "-"*60)
print(f" CPP ANALYSIS: ODD-DEGREE NODES ({len(odd_verts)})")
print("-" * 60)
print(f" Odd-degree vertices: {odd_verts}\n")

# Computation of the pairwise distance matrix using Dijkstra's algorithm
n_odd = len(odd_verts)
dist_matrix = np.zeros((n_odd, n_odd))

for i, u in enumerate(odd_verts):
    # Single-source Dijkstra for current odd node to all other nodes
    lengths = nx.single_source_dijkstra_path_length(G, u, weight="weight")
    for j, v in enumerate(odd_verts):
        dist_matrix[i, j] = lengths[v]

# Matrix Validation Outputs
print(f" Distance matrix dimensions : {dist_matrix.shape}")
print(f" Symmetry validation        : {np.allclose(dist_matrix, dist_matrix.T)}")
print(f" Null diagonal validation   : {np.all(np.diag(dist_matrix) == 0)}")
print(f" Minimum path distance      : {np.min(dist_matrix[dist_matrix > 0]):.2f} meters")
print(f" Maximum path distance      : {np.max(dist_matrix):.2f} meters")

# Visual representation of a 5x5 sub-matrix (Top-left corner)
print(f"\nMatrix Preview (Top-left 5x5 corner):")
print(f"{' ':>4}", end="")
for v in odd_verts[:5]:
    print(f" v{v:>2}", end="")
print()

for i in range(5):
    print(f"v{odd_verts[i]:>2} ", end="")
    for j in range(5):
        print(f" {dist_matrix[i,j]:>5.1f}", end="")
    print()